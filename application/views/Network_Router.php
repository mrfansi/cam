<div class="container">
  <?php
    $action_detail  = base_url('manage/router/detail/');
    $action_hapus   = base_url('manage/router/hapus/');
  ?>
  ...
  <script type="text/javascript">
    ...
    function baru() {
      ...
      // show body
      body.load( "<?php echo base_url('manage/router/tambah'); ?> #post-data" );
    }
    ...
    function detil(data) {
      ...
      // show body
      body.load( "<?php echo base_url('manage/router/detail/'); ?>" + id + " #post-data" );
    }
  ...
  <div class="field">
    <p class="control is-expanded">
      <input id="filter" class="input" type="text" name="filter" placeholder="Cari data Router disini..." autofocus="yes">
    </p>
  </div>
    ...
    <table class="table is-striped is-narrow is-fullwidth is-hoverable" id="data-table">
      ...
      <tbody>
        ...
        <?php foreach ($all_records as $data): ?>
          <tr <?= $data->status_router == false ? 'class="is-down" id="data-row"' : '' ;?>>
            <td><?php echo $data->hostname_router; ?></td>
            <td><?php echo $data->ip_addr_router; ?></td>
            <td><?php echo $data->tipe_router; ?></td>
            <td><?php echo $data->jenis_router; ?></td>
            <td><?php echo $data->pop->nama_pop; ?></td>
            <td id="status" class="<?= $data->status_router == false ? 'down' : 'up' ?>"><?= $data->status_router == false ? 'Down' : 'Up'; ?></td>
            <td>
              <button class="button is-small is-info" data="<?php echo $data->kode_router; ?>" onclick="detil($(this))">Lihat Detil</button>
              <button class="button is-small is-info" data="<?php echo $data->kode_router; ?>">Terminal</button>
              <button class="button is-small is-info" data="<?php echo $data->kode_router; ?>" onclick="hapus($(this))">Hapus</button>
              <button class="button is-small is-info" data="<?php echo $data->kode_router; ?>">Log</button>
            </td>
          </tr>
        <?php endforeach; ?>
        <?php else: ?>
...
<div class="modal" id="modal-crud">
  ...
  <div class="modal-card">
    <header class="modal-card-head">
      <p class="modal-card-title">Data Router</p>
  ...
  $(document).ready(function(){
    ...
    $(function() {
      setInterval(function(){
        if (update) {
          $( "#data" ).load( "<?php echo base_url('manage/router'); ?> #data-table" );
        }
